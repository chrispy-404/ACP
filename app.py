...
# --- 4. DATENBANK VERBINDUNG (MySQL) ---
def get_db_connection():
    # Wir nutzen st.secrets für die sichere Verbindung in der Cloud
    # Fallback für lokalen Test, falls keine Secrets da sind (bitte Daten anpassen für lokal!)
    try:
        if "mysql" in st.secrets:
            # Verbindung über Streamlit Secrets (in der Cloud)
            ...
            # Fallback (nur zum Testen, wenn secrets.toml fehlt)
            conn = mysql.connector.connect(
                host='database-5019192732.webspace-host.com', # Dein Strato Host
                user='dbu3967599',                            # Dein Strato User
                password='DEIN_PASSWORT_HIER',                # Dein Passwort (nur lokal ändern!)
                database='dbs15069082'                        # Deine DB
            )
        return conn
